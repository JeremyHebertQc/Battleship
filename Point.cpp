...
void Point::read(std::istream& flux)
{
	...
	flux >> trash >> _x >> trash >> _y >> trash >> _color;
}

// Surcharge des opérateurs
const Point& Point::operator=(const Point& otherPoint)
{
	_x = otherPoint._x;
	_y = otherPoint._y;
	_color = otherPoint._color;
...
bool Point::operator>=(const Point& otherPoint) const
{
	return operator>(otherPoint) || operator==(otherPoint);
}
...
{
	return Point(_x + otherPoint._x, _y + otherPoint._y);
}
